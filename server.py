...
def migrate_json_to_db():
    ...
    if row and row['value'] == 'true': conn.close(); return
    files = [f for f in os.listdir(TMDB_CACHE_DIR) if f.endswith(".json")]
    if not files: conn.close(); return
    ...
    for idx, f in enumerate(files):
        ...
        if (idx + 1) % 2000 == 0: conn.commit(); log(f"  🚚 이관 진행 중... ({idx+1}/{len(files)})")
    conn.execute("INSERT OR REPLACE INTO server_config (key, value) VALUES ('json_migration_done', 'true')")
    conn.commit(); conn.close(); log("✅ [MIGRATE] JSON 캐시 이관 완료")
...
def clean_title_complex(title):
    ...
    orig_title = nfc(title); cleaned = REGEX_EXT.sub('', orig_title); cleaned = REGEX_CH_PREFIX.sub('', cleaned); cleaned = REGEX_TMDB_HINT.sub('', cleaned)
    ...
    cleaned = REGEX_DATE_YYMMDD.sub(' ', cleaned); year_match = REGEX_YEAR.search(cleaned)
    year = year_match.group().replace('(', '').replace(')', '') if year_match else None
    cleaned = REGEX_YEAR.sub(' ', cleaned); cleaned = REGEX_BRACKETS.sub(' ', cleaned); cleaned = cleaned.replace("(자막)", "").replace("(더빙)", "").replace("[자막]", "").replace("[더빙]", "")
    cleaned = REGEX_JUNK_KEYWORDS.sub(' ', cleaned); cleaned = REGEX_SPECIAL_CHARS.sub(' ', cleaned); cleaned = REGEX_LEADING_INDEX.sub('', cleaned)
    cleaned = re.sub(r'([가-힣a-zA-Z])(\d+)$', r'\1 \2', cleaned); cleaned = REGEX_SPACES.sub(' ', cleaned).strip()
    if len(cleaned) < 2:
...
def get_tmdb_info_server(title, ignore_cache=False):
    ...
    headers = {"Authorization": f"Bearer {TMDB_API_KEY}"}; base_params = {"include_adult": "true", "region": "KR"}
    ...
    try:
        ...
        if results:
            ...
            cast_data = d_resp.get('credits', {}).get('cast', []); actors = [{"name": c['name'], "profile": c['profile_path'], "role": c['character']} for c in cast_data[:10]]
            crew_data = d_resp.get('credits', {}).get('crew', []); director = next((c['name'] for c in crew_data if c.get('job') == 'Director'), "")
            info = {"tmdbId": f"{m_type}:{t_id}", "genreIds": [g['id'] for g in d_resp.get('genres', [])] if d_resp.get('genres') else [], "genreNames": genre_names, "director": director, "actors": actors, "posterPath": d_resp.get('poster_path'), "year": yv, "overview": d_resp.get('overview'), "rating": rating, "seasonCount": d_resp.get('number_of_seasons'), "failed": False}
            if m_type == 'tv':
                info['seasons_data'] = {}
                for s_num in range(1, (d_resp.get('number_of_seasons') or 1) + 1):
                    ...
                    if 'episodes' in s_resp:
                        for ep in s_resp['episodes']: info['seasons_data'][f"{s_num}_{ep['episode_number']}"] = {"overview": ep.get('overview'), "air_date": ep.get('air_date')}
...
def scan_recursive_to_db(bp, prefix, category):
    ...
    for idx, fp in enumerate(all_files):
        ...
        if mid not in db_data: cursor.execute('INSERT OR REPLACE INTO episodes (id, series_path, title, videoUrl, thumbnailUrl) VALUES (?, ?, ?, ?, ?)', (mid, spath, os.path.basename(fp), f"/video_serve?type={prefix}&path={urllib.parse.quote(rel)}", f"/thumb_serve?type={prefix}&id={mid}&path={urllib.parse.quote(rel)}"))
...
def perform_full_scan():
    ...
    for label, ck in pk:
        ...
        if os.path.exists(path):
            ...
            conn = get_db(); conn.execute("INSERT OR REPLACE INTO server_config (key, value) VALUES (?, 'true')", (f'scan_done_{ck}',)); conn.commit(); conn.close(); build_all_caches()
...
def fetch_metadata_async(force_all=False):
    ...
    try:
        ...
        all_names_rows = conn.execute('SELECT name FROM series WHERE tmdbId IS NULL AND failed = 0').fetchall(); conn.close()
        if not all_names_rows: log("📊 대상 작품 없음 (이미 모든 파일이 매칭됨)"); IS_METADATA_RUNNING = False; return
        name_groups = {}
        for row in all_names_rows:
            orig_name = row['name']; ct, year = clean_title_complex(orig_name)
            if not ct: continue
            key = (ct, year); name_groups.setdefault(key, set()).add(orig_name)
        tasks = [{'clean_title': ct, 'year': year, 'sample_name': list(names)[0], 'orig_names': list(names)} for (ct, year), names in name_groups.items()]
        total = len(tasks); log(f"📊 대상 작품: {total}개 (중복 제외 완료, 총 {len(all_names_rows)}개 파일)")
        def process_one(task): info = get_tmdb_info_server(task['sample_name'], ignore_cache=force_all); return (task, info)
        batch_size = 50
        for i in range(0, total, batch_size):
            batch = tasks[i:i+batch_size]; results = []
            with ThreadPoolExecutor(max_workers=10) as executor:
                future_to_task = {executor.submit(process_one, t): t for t in batch}
                for future in as_completed(future_to_task): results.append(future.result())
            conn = get_db(); cursor = conn.cursor()
            for task, info in results:
                ...
                if info.get('failed'):
                    ...
                    log(f"❌ 매칭 실패: {task['sample_name']} (정제: {task['clean_title']})")
                else:
                    up = (info.get('posterPath'), info.get('year'), info.get('overview'), info.get('rating'), info.get('seasonCount'), json.dumps(info.get('genreIds', [])), json.dumps(info.get('genreNames', []), ensure_ascii=False), info.get('director'), json.dumps(info.get('actors', []), ensure_ascii=False), info.get('tmdbId'))
                    ...
                    if 'seasons_data' in info:
                        def chunked_ep_fetch(names):
                            ...
                            for j in range(0, len(names), 900):
                                chunk = names[j:j+900]; ps = ','.join(['?'] * len(chunk)); cursor.execute(f'SELECT id, title FROM episodes WHERE series_path IN (SELECT path FROM series WHERE name IN ({ps}))', chunk); all_eps.extend(cursor.fetchall())
                            return all_eps
                        eps_to_update = chunked_ep_fetch(orig_names); ep_batch = []
                        for ep_row in eps_to_update:
                            sn, en = extract_episode_numbers(ep_row['title'])
                            if en:
                                ei = info['seasons_data'].get(f"{sn}_{en}")
                                if ei: ep_batch.append((ei.get('overview'), ei.get('air_date'), sn, en, ep_row['id']))
                        if ep_batch: cursor.executemany('UPDATE episodes SET overview=?, air_date=?, season_number=?, episode_number=? WHERE id=?', ep_batch)
                    log(f"✅ 매칭 성공: {task['clean_title']} ({len(orig_names)}개 파일)")
            conn.commit(); conn.close(); log(f"🔍 [{round((i+len(batch))/total*100, 1)}%] 매칭 진행 중 ({i+len(batch)}/{total})")
            if (i // batch_size) % 2 == 0: build_all_caches()
        log("✅ 모든 메타데이터 고도화 및 일괄 업데이트 완료")
...
def get_category_sections():
    cat = request.args.get('cat', 'movies'); return jsonify(get_sections_for_category(cat))
...
@app.route('/list')
def get_list():
    p = request.args.get('path', ''); m = {"영화": "movies", "외국TV": "foreigntv", "국내TV": "koreantv", "애니메이션": "animations_all", "방송중": "air", "movies": "movies", "foreigntv": "foreigntv", "koreantv": "koreantv", "animations_all": "animations_all", "air": "air"}
    kw = request.args.get('keyword'); cat = m.get(p)
    if not cat: return jsonify([])
    bl = _FAST_CATEGORY_CACHE.get(cat, []); lim = int(request.args.get('limit', 500)); off = int(request.args.get('offset', 0))
...
def get_series_detail_api():
    ...
    conn = get_db(); row = conn.execute('SELECT * FROM series WHERE path = ?', (path,)).fetchone()
    ...
    if series.get('tmdbId'): cursor = conn.execute("SELECT e.* FROM episodes e JOIN series s ON e.series_path = s.path WHERE s.tmdbId = ?", (series['tmdbId'],))
    else: cursor = conn.execute("SELECT * FROM episodes WHERE series_path = ?", (path,))
    eps = []; seen = set()
    for r in cursor.fetchall():
        if r['videoUrl'] not in seen: eps.append(dict(r)); seen.add(r['videoUrl'])
    series['movies'] = sorted(eps, key=lambda x: natural_sort_key(x['title'])); conn.close(); _DETAIL_CACHE.append((path, series))
...
def search_videos():
    ...
    conn = get_db(); cursor = conn.execute('SELECT s.*, e.id as ep_id, e.videoUrl, e.thumbnailUrl, e.title FROM series s LEFT JOIN episodes e ON s.path = e.series_path WHERE (s.path LIKE ? OR s.name LIKE ?) GROUP BY s.path ORDER BY s.name ASC', (f'%{q}%', f'%{q}%'))
    rows = []
...
def build_all_caches(): build_home_recommend(); _rebuild_fast_memory_cache()
def _rebuild_fast_memory_cache():
    ...
    for cat in ["movies", "foreigntv", "koreantv", "animations_all", "air"]:
        ...
        for row in all_rows:
            ...
            # 혁신: tmdbId가 있으면 tmdbId로, 없으면 정제된 제목(ct)과 연도로 그룹화
            group_key = (item['tmdbId'], ct, year) if item['tmdbId'] else (ct, year)
            if group_key not in rows_dict:
                ...
                if item.get('ep_id'):
                    rows_dict[group_key]['movies'].append({"id": item.pop('ep_id'), "videoUrl": item.pop('videoUrl'), "thumbnailUrl": item.pop('thumbnailUrl'), "title": item.pop('title')})
        temp[cat] = list(rows_dict.values())
    conn.close(); _FAST_CATEGORY_CACHE = temp

def build_home_recommend():
    global HOME_RECOMMEND
    try:
        m = _FAST_CATEGORY_CACHE.get('movies', []); k = _FAST_CATEGORY_CACHE.get('koreantv', []); a = _FAST_CATEGORY_CACHE.get('air', [])
        HOME_RECOMMEND = [{"title": "지금 가장 핫한 인기작", "items": random.sample(m + k, min(20, len(m+k))) if m+k else []}, {"title": "실시간 방영 중", "items": a[:20]}]
    except: pass

def report_db_status():
    try:
        conn = get_db(); eps = conn.execute("SELECT COUNT(*) FROM episodes").fetchone()[0]; ser = conn.execute("SELECT COUNT(*) FROM series").fetchone()[0]; mtch = conn.execute("SELECT COUNT(*) FROM series WHERE tmdbId IS NOT NULL").fetchone()[0]; cch = conn.execute("SELECT COUNT(*) FROM tmdb_cache").fetchone()[0]
        log("="*50); log(f"📊 [DB STATUS] 파일: {eps} / 시리즈: {ser} / 매칭: {mtch} / 캐시: {cch}"); log("="*50); conn.close()
    except: pass

if __name__ == '__main__':
    init_db(); migrate_json_to_db(); report_db_status(); threading.Thread(target=build_all_caches, daemon=True).start()
