...
suspend fun processThemedSections(result: List<Series>): List<ThemeSection> {
    ...
    // 1. 방금 업데이트된 신작 (상위 40개로 확장)
    val newArrivals = distinctResult.take(40)
    ...
    if (tA.isNotEmpty()) sectionsList.add(ThemeSection("action", "박진감 넘치는 액션 & 어드벤처", tA.take(60)))
    if (tF.isNotEmpty()) sectionsList.add(ThemeSection("fantasy", "판타지 & SF", tF.take(60)))
    if (tC.isNotEmpty()) sectionsList.add(ThemeSection("comedy", "코미디 & 라이프", tC.take(60)))
    if (tT.isNotEmpty()) sectionsList.add(ThemeSection("thriller", "미스터리 & 스릴러", tT.take(60)))
    if (tR.isNotEmpty()) sectionsList.add(ThemeSection("romance", "로맨스 & 드라마", tR.take(60)))
    if (tM.isNotEmpty()) sectionsList.add(ThemeSection("family", "가족과 함께", tM.take(60)))
    
    // 마지막에 나머지 전체 목록 추가 (중복 없이)
    if (remainingPool.isNotEmpty()) {
        sectionsList.add(ThemeSection("all_remaining", "전체 목록", remainingPool.take(400)))
    }
